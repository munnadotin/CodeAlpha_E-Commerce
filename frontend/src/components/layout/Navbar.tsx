import { Handbag, Search, User2 } from 'lucide-react';
import '../../App.css';
import { Link } from 'react-router-dom';

const Navbar = () => {
  return (
    <div className="w-full sticky top-0 z-50 border-b border-slate-300 bg-white shadow-xs">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-8 md:gap-12 p-4">
        {/* Logo */}
        <Link to={'/'} className="text-2xl font-normal tracking-wider text-gray-900 cursor-pointer">ALPHA<span className="font-light text-gray-500">STORE</span></Link>
        ...
        <div className="relative flex items-center gap-4">
          ...
          <button className='flex items-center gap-2 px-4 py-1.5 border border-slate-300 rounded-md shadow-xs cursor-pointer'>
            <User2 strokeWidth={1.5} className='h-5 w-5' /> Sign In / Register
          </button>
        </div>
      </div>
    </div>
